...
import be.atbash.config.mp.converter.testclass.ToNullTestConverter;
...
class ConvertValueUtilTest {
    ...
    void convertValue_convertToNull() {
        ...
        Assertions.assertThat(exception.getMessage()).isEqualTo("MPCONFIG-141: The config property 'test' with the config value 'Atbash' was converted to null from the following Converter: be.atbash.config.mp.converter.testclass.ToNullTestConverter");
    }
    ...
    void convertValue_convertToEmpty() {
        NoSuchElementException exception = Assertions.catchThrowableOfType(() ->
                ...
                , NoSuchElementException.class);
        Assertions.assertThat(exception.getMessage()).isEqualTo("MPCONFIG-140: The config property 'test' is defined as the empty String (\"\") which the following Converter considered to be null: be.atbash.config.mp.converter.Converters$BuiltInConverter");
    }
